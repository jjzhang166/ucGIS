...
{
	AreaIndex areaindex; 
	if(pareaindexarray->InitState== GUI_ARRAY_STATE_NOT_CREATED)
	  GUI_ARRAY_CREATE(pareaindexarray);
    memset(&areaindex,0,sizeof(AreaIndex));
	fread(&areaindex.mapExtentRect.fMinX,sizeof(float),1,fp);
	fread(&areaindex.mapExtentRect.fMinY,sizeof(float),1,fp);
	fread(&areaindex.mapExtentRect.fMaxX,sizeof(float),1,fp);
	fread(&areaindex.mapExtentRect.fMaxY,sizeof(float),1,fp);
	fread(&areaindex.offset,sizeof(int),1,fp);
    GUI_ARRAY_AddItem(pareaindexarray,&areaindex,sizeof(AreaIndex));
}
GRIDINDEX_EXT BOOLEAN SwitchPointtoArea(float lon,float lat,GUI_ARRAY* pareaindexarray)
{
	BOOLEAN brtCode=FALSE;
	return brtCode;
}

...
BEGIN_MESSAGE_MAP(CMapToolView, CView)
	...
	ON_COMMAND(ID_BUILDAREAINDEX, &CMapToolView::OnBuildareaindex)
	ON_COMMAND(ID_SAVEAREAINDEX, &CMapToolView::OnSaveareaindex)
END_MESSAGE_MAP()
...
void CMapToolView::OnOpenQm()
{
	...
	if(m_strFilePath.GetLength()>0)
	{
		...
		m_Doc.m_FileMap.ReleaseMap();  // 释放曾打开的地图
...
void CMapToolView::OnBuildareaindex()
{
	m_Doc.m_FileMap.BuildAreaIndex();
}

void CMapToolView::OnSaveareaindex()
{
	// TODO: 在此添加命令处理程序代码
	CFileDialog myDlg(TRUE,_T(".inx"),_T("*.inx"),OFN_PATHMUSTEXIST|OFN_OVERWRITEPROMPT,_T("Txt Files (*.inx)|*.inx| All Files (*.*)|*.*||"),NULL);
	CString m_strFilePath;
	...
	if(m_strFilePath.GetLength()>0)
	{
		CString strPath = m_strFilePath;
		TCHAR* szPath =  strPath.GetBuffer() ;
		m_Doc.m_FileMap.SaveAreaIndex(szPath);
	}
}

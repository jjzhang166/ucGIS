...
void CMyMapDoc::ReleaseMap()
{
	...
	m_LayerVector.clear();
	ReleaseGridIndex();
	ReleaseGridIndexIndex();
	ReleaseAreaIndex();
}
void CMyMapDoc::ReleaseAreaIndex()
{
	int nCount = careaindexvector.size();
	CAreaIndex *pArea;
	for(int i=0;i<nCount;i++)
	{
      pArea=(CAreaIndex*)careaindexvector[i];
	  if(pArea)
	  {
		  pArea->~CAreaIndex();
	      delete pArea;
	...
	careaindexvector.clear();

}
void CMyMapDoc::ReleaseGridIndex()
{
...
void CMyMapDoc::ReleaseGridIndexIndex()
{
...
void CMyMapDoc::SaveGridIndex(TCHAR* pszFileName)
{
	...
	if(!fp)	return;
	int i=0,offset=0;
	int fileid=INDEXFILEID;
	//fwrite(&fileid,sizeof(int),1,fp);  //
	//offset+=sizeof(int);
...
void CMyMapDoc::ReadGridIndex(TCHAR* pszFileName)
{
	...
	fp = fopen(path,"rb");
	if(!fp)	return;
//	fread(&indexfileid,sizeof(int),1,fp);  //
//    if(INDEXFILEID!=indexfileid) return;
...
void CMyMapDoc::BuildAreaIndex()
{
	...
	for(int i=0;i<nCount;i++)
	{
		CLayer* layer = ((CLayer*)m_LayerVector[i]);
		layer->m_mapCoordMap = m_mapCoordMap;
		layer->BuildAreaIndex(careaindexvector);
	}	
	m_bAreaIndexHaveBuild=true;

}
void CMyMapDoc::SaveAreaIndex(TCHAR* pszFileName)
{
	if (m_bAreaIndexHaveBuild)
	{
	SetCursor (LoadCursor (NULL, IDC_WAIT));
	char path[255];
	WToA(pszFileName,path);
	...
	fp = fopen(path,"wb");
	if(!fp)	return;
	int fileid=AREAINDEXFILEID;
	fwrite(&fileid,sizeof(int),1,fp);  //
	int indexNum=(int)careaindexvector.size();

	fwrite(&indexNum,sizeof(int),1,fp);  //
	for ( int i=0;i<indexNum;i++)
	{

		CAreaIndex* pcareaindex;
		pcareaindex=careaindexvector[i];
		pcareaindex->WriteToFile(fp);
	}
	fclose(fp);
	}
}

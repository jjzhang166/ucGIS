...
CAreaIndex::CAreaIndex()
{
}
//CAreaIndex::CAreaIndex(float fminx,float fminy,float fmaxx,float fmaxy,int offset,TCHAR *areaname)
CAreaIndex::CAreaIndex(float fminx,float fminy,float fmaxx,float fmaxy,int offset)//,TCHAR *areaname)
{
	mapExtentRect.fMaxX=fmaxx;
	mapExtentRect.fMinX=fminx;
	mapExtentRect.fMaxY=fmaxy;
	mapExtentRect.fMinY=fminy;
	m_offset=offset;
...
void CAreaIndex::WriteToFile(FILE *fp)
{
	fwrite(&mapExtentRect.fMinX,sizeof(float),1,fp);
	fwrite(&mapExtentRect.fMinY,sizeof(float),1,fp);
	fwrite(&mapExtentRect.fMaxX,sizeof(float),1,fp);
	fwrite(&mapExtentRect.fMaxY,sizeof(float),1,fp);
	fwrite(&m_offset,sizeof(int),1,fp);
}

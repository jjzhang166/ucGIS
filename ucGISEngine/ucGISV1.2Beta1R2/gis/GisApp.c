...
static char  psmapfile[100];
static char  psmapinxfile[100];
static char  psmapindfile[100];
static char  startmappath[100];
static char  psmapchfile[100];
static char  gSearchKeystr[100];
static char  *psmappath;
...
#define MAXSEARCHINDEX 100
MAPSearchIndex MymapsearchIndex[MAXSEARCHINDEX];
  ...
  case WM_NOTIFY_PARENT:
    ...
    case WM_NOTIFICATION_RELEASED:
		...
		case GUI_ID_BUTTON4:
            ...
            SearchMapUseIndex(psmapchfile,&myUcMap,aBuffer, MymapsearchIndex, &nCount,MAXSEARCHINDEX);
		    _deletallitem(hListBox);
			for(i=0;i<nCount;i++)
			{
				LISTBOX_AddString(hListBox,MymapsearchIndex[i].name);
			}
            break;
		case GUI_ID_LISTBOX0:
			
			i = LISTBOX_GetSel(hListBox);
			MapGotoSearch(psmapfile,&m_mapCoordMap,MymapsearchIndex[i]);
...
static void _createSearchDialog()
{
  ...
  memset(&gSearchKeystr,0,100);
  sprintf(gSearchKeystr,"%s","清水河");
  EDIT_SetText(hEdit, gSearchKeystr);
 ...
 // UI_KeyBoard_CreateWindows(gSearchKeystr,100,LCD_GetXSize(),LCD_GetYSize()/2);
}
...
//          unsigned long   style;
          ...
          //index
		  strcat(currpath2,"*.inx");  
          handle1=_findfirst(currpath2,&filestruct1);   
		  if(handle1==-1)   return NULL;         //如果handle为-1，表示当前目录为空，则结束查找而返回   
		  sprintf(warn,"Read %s ...",filestruct1.name);
		  sprintf(startmappath,"%s%s",filepath1,filestruct1.name);
          //indexindex
		  strcat(currpath3,"*.ind");  
          handle1=_findfirst(currpath3,&filestruct1);   
		  if(handle1==-1)   return NULL;         //如果handle为-1，表示当前目录为空，则结束查找而返回   
		  sprintf(warn,"Read %s ...",filestruct1.name);
		  sprintf(currpath3,"%s%s",filepath1,filestruct1.name);
...
//  char * mapfile;
  ...
  case WM_CREATE:
	...
	//	_MoveMap();
		...
		if(!AutoSwitchMap(&myUcMap,startmappath,start_lon,start_lat))
		{
		   memset(psmapfile,0,100);
		   FieldVar(startmappath,0,'.',psmapfile);
		   sprintf(psmapinxfile,"%s.inx",psmapfile);
		   sprintf(psmapindfile,"%s.ind",psmapfile);
		   sprintf(psmapchfile,"%s.sch",psmapfile);
           strcat(psmapfile,".qm2");
		}
		Map_ReadFileIndex(&myUcMap,psmapfile);
        ...
        Map_ReadFileIndexIndex(&myUcMap,psmapinxfile);
        Map_ReadGridIndexWithIndex(&myUcMap,psmapindfile);
  ...
  case WM_PAINT:
		 ...
		 GUI_Clear();
#if !(MAPNOGRIDINDEX)		
         //Map_ReadFileIndexIndex(&myUcMap,currpath3);
         Map_ReadGridIndexWithIndex(&myUcMap,psmapindfile);
		 Map_ReadMapWithGridIndex(&myUcMap,psmapfile);
#endif
		myUcMap.pmapCoorMap=&m_mapCoordMap;
...
char  UI_gisSDK(char *filepath, INT16U SizeX, INT16U SizeY, GUI_FONT GUI_UNI_PTR * pfont, GpsDataInfo * Paramer)
{
	...
	if(filepath)
       sprintf(startmappath,"%s\\地级市区界.inx",filepath);
	else
	   GetMapFileName();//return -1;
	
	//Paramer->m_xx=104.09539;
	//Paramer->m_yy=30.627087;
	start_lon=Paramer->m_xx;
    start_lat=Paramer->m_yy;
	//GetMapFileName();//return -1;
//    psmapfile=currpath1;
	bGisExit=FALSE;
	//GUI_UC_SetEncodeGBK();
